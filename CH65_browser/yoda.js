...
var annotate_variants = {original: 'UCA860',
                         variant: 'CH65'};
...
var main_svg;
var use_data;
var kd_data = {'Expression': {}, 'MA90': {}, 'SI06': {}, 'G189E': {}};
var kd_conc_map = {
  'Expression': null,
  'MA90': [0, -12, -11.25, -10.5, -9.75, -9, -8.25,-7.5, -6.75, -6],
  'SI06':[0, -12, -11.25, -10.5, -9.75, -9, -8.25,-7.5, -6.75, -6],
  'G189E': [0, -12, -11.25, -10.5, -9.75, -9, -8.25,-7.5, -6.75, -6]
}
...
var scale_y_violin =  {'Expression': 0.1, 'MA90': 0.1, 'SI06': 0.1,
                       'G189E': 0.1};
...
function setup_interaction() {
  ...
  */
  ...
  main_svg.append('text')
    ...
    .attr('x', xs(data_by_variant[original]['fdl_x'])-0)
    .attr('y', 300)
  ...
  main_svg.append('line')
    ...
    .attr('stroke', '#555555')
    .attr('stroke-width', 1)
    .attr('x1', xs(data_by_variant[original]['fdl_x'])+10)
    .attr('y1', 320)
  ...
  main_svg.append('line')
    ...
    .attr('stroke', '#555555')
    .attr('stroke-width', 1)
    .attr('x1', xs(data_by_variant[variant]['fdl_x'])+50)
    .attr('y1', 110)
...
function kd_for(kd_var_tmp) {
  kd_var = kd_var_tmp;
  if (kd_var == 'MA90') {
    x_by_kd = d3.scaleLinear().domain([7.5,10.5]).range([630,790]);
    color_by_kd = d3.scaleSequential(d3.interpolateViridis).domain([7.5,10.5]);
    d3.select("#x_axis_label_violin").text("-log10(Kd)");
  }
  else if (kd_var == 'G189E') {
    x_by_kd = d3.scaleLinear().domain([6,10]).range([630,790]);
    color_by_kd = d3.scaleSequential(d3.interpolateViridis).domain([6,10]);
    d3.select("#x_axis_label_violin").text("-log10(Kd)");
}
...
function read_files(fpath) {
  ...
  d3.csv(fpath).then(function(data) {
    ...
    aq.loadArrow('data/MA90.arrow').then((td) => kd_data['MA90']=td);
    aq.loadArrow('data/G189E.arrow').then((td) => kd_data['G189E']=td);
    aq.loadArrow('data/SI06.arrow').then((td) => kd_data['SI06']=td);
